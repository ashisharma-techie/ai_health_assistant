...
def render_tab():
    ...
    # First try .env, then try Streamlit secrets, otherwise show message
    ...
    if not locationiq_key:
        st.error("LOCATIONIQ_KEY not found.")
        st.info("Create a .env file in your project root and add: LOCATIONIQ_KEY=your_key_here")
        return

    location = st.text_input(
        "Enter your area, locality, or city",
    ...
    if st.button("Search Nearby Doctors") and location:
        with st.spinner("Searching..."):
            try:
                ...
                if not results:
                    st.warning("No doctors or clinics found nearby, even within 20 km. Try a nearby larger town or city.")
                else:
                    km = used_radius / 1000
                    st.success(f"Found {len(results)} nearby doctors or clinics within {km:.0f} km")
